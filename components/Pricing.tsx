...
const Pricing: React.FC = () => {
  ...
  const handleMouseLeave = (e: React.MouseEvent<HTMLDivElement>, isFeatured: boolean) => {
    const card = e.currentTarget;
    const baseScale = isFeatured ? (window.innerWidth >= 1024 ? 1.05 : 1) : 1;
  ...
  const plans = [
    {
      ...
      desc: 'Establish your brand presence.',
    ...
    },
    {
      ...
      desc: 'Accelerate with consistency.',
  ...
  return (
    <section id="investment" className="py-24 md:py-32 px-6 bg-[#111] text-white overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 md:mb-16">
          <p className="text-blue-400 font-bold text-xs tracking-widest uppercase mb-4">Velocity</p>
          <h2 className="text-3xl md:text-5xl font-heading font-extrabold tracking-tight mb-8 uppercase">Marketing Plans</h2>
          
          <div className="inline-flex p-1 bg-white/5 rounded-full border border-white/10">
            <button 
              onClick={() => setIsAnnual(false)}
              className={`px-6 md:px-8 py-2 rounded-full text-[10px] md:text-xs font-bold transition-all ${!isAnnual ? 'bg-white text-black shadow-lg' : 'text-gray-400'}`}
            >
              Monthly
            </button>
            <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan, idx) => (
            <div 
              ...
              className={`relative p-8 md:p-10 rounded-2xl border flex flex-col transition-all duration-300 ${
                plan.isLight 
                ? 'bg-white text-black lg:scale-105 z-10 border-blue-500 shadow-2xl' 
                : 'bg-transparent text-white border-white/10'
              }`}
            >
              {plan.popular && (
                <div className="absolute top-6 right-6 bg-[#ff0055] text-white text-[8px] font-black px-2 py-1 rounded">
                  MOST POPULAR
                </div>
              )}
              ...
              <div className="text-4xl md:text-5xl font-heading font-bold mb-4 flex items-baseline">
                <span className="text-2xl mr-1">₹</span>{plan.price}<span className="text-sm font-medium opacity-60 ml-2">/ {isAnnual ? 'yr' : 'mo'}</span>
              </div>
              ...
              <ul className="flex flex-col gap-4 mb-10 flex-grow">
                {plan.features.map((f, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm font-semibold">
                    <CheckCircle2 size={16} className={`mt-0.5 flex-shrink-0 ${plan.isLight ? 'text-blue-600' : 'text-blue-400'}`} />
                    <span>{f}</span>
                  </li>
                ))}
              </ul>
              <a 
                ...
                className={`w-full py-4 text-center font-black uppercase text-[10px] tracking-widest rounded-xl transition-all ${
                  plan.isLight ? 'bg-black text-white hover:bg-gray-800' : 'bg-white text-black hover:bg-gray-200'
                }`}
              >
                Choose Plan
              </a>
